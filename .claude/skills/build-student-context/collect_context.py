...
def check_src_changes(root: Path) -> bool:
    ...
    try:
        ...
        # Also check untracked non-.py files or new .py files
        result2 = subprocess.run(
            ["git", "status", "--porcelain", "src/"],
            capture_output=True, text=True, cwd=root
        )
        if result2.returncode == 0 and result2.stdout.strip():
            return False  # There are untracked/modified files
        return True  # No changes detected
    except Exception:
        return True  # Can't determine, assume no changes
...
def collect_past_plans(root: Path) -> list[str]:
    ...
    if not plans_dir.is_dir():
        return []
    return sorted([f.name for f in plans_dir.iterdir() if f.is_file()])
